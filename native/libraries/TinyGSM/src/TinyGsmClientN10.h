/**
 * @file       TinyGsmClientN10.h
 * @author     ayx
 */

#ifndef TinyGsmClientN10_h
#define TinyGsmClientN10_h
...
//============================================================================//
//============================================================================//
//                    Declaration of the TinyGsmN10 Class
//============================================================================//
//============================================================================//
...
class TinyGsmN10
{
	...
	{
		friend class TinyGsmN10;
		typedef TinyGsmFifo<uint8_t, TINY_GSM_RX_BUFFER> RxFifo;

	public:
		GsmClient() {}

		GsmClient(TinyGsmN10& modem, uint8_t mux = 1) {
			init(&modem, mux);
		}

		bool init(TinyGsmN10* modem, uint8_t mux = 1) {
			this->at = modem;
			this->mux = mux;
	...
	public:
		virtual int connect(const char *host, uint16_t port) {
			...
			sock_connected = at->modemConnect(host, port, 1); // Single Connection Only
	...
	public :
	  ...
	  TinyGsmN10(Stream& stream = GSM_DEFAULT_STREAM)
#else
	    TinyGsmN10(Stream& stream)
#endif
		: stream(stream)
	{
	...
	bool factoryDefault() {
		...
		waitResponse();
		//sendAT(GF("+ICF=3,1")); // 8 data 0 parity 1 stop   //commented by ayz
		//waitResponse();                                     //commented by ayz
		sendAT(GF("+ENPWRSAVE=0"));  // Disable PWR save
		waitResponse();
		//sendAT(GF("+XISP=0"));  // Use internal stack       //commented by ayz
		//waitResponse();                                     //commented by ayz
	...
	bool restart() {
		...
		//sendAT(GF("+CFUN=15"));        // commented out by ayz
		sendAT(GF("+CFUN=1,1"));          // added by ayz
	...
	String getIMEI() {
		//sendAT(GF("+GSN"));          //commented by ayz
		sendAT(GF("+CGSN"));
	...
	bool gprsConnect(const char* apn, const char* user = NULL, const char* pwd = NULL) {
		...
		//sendAT(GF("+XISP=0"));  //commented out by ayz
		//waitResponse();         //commented out by ayz
		...
		do {
			...
			while (stream.available() > 0) {
				...
				// }
					...
					// if(sockets[1]->sock_connected){
						sockets[1]->rx.put(stream.read());
						data = "";
					// }
				}
			}
