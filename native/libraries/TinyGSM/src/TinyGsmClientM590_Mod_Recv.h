/**
 * @file       TinyGsmClientM590_Mod_Recv.h
 * @author     Shadly Salahuddin
 */

#ifndef TinyGsmClientM590_Mod_Recv_h
#define TinyGsmClientM590_Mod_Recv_h
...
const char TCP_CLOSE[]              PROGMEM  =  "AT+TCPCLOSE=0" ; /*!< GSM AT Command : Close TCP connection*/
...
class GsmClient : public Client{
  ...
  public:
    ...
    virtual uint8_t connected() {
      if(this->gsm_modem->modemConnected()){
        while(this->_uart_stream->available()){
          if(this->_tcp_bytes_pending){
            rx.put(stream.read());
            this->_tcp_bytes_pending = this->_tcp_bytes_pending - 1;
          } else{
            _tcp_buffer[_tcp_buffer_write_index]= (char)this->_uart_stream->read();
            _tcp_buffer_write_index = _tcp_buffer_write_index + 1;
            if(findString(_tcp_buffer, F("AT+TCPRECV:"), _tcp_buffer_write_index)){
              int mux = stream.readStringUntil(',').toInt();
              _tcp_bytes_pending = stream.readStringUntil(',').toInt();
              memset(_tcp_buffer, 0x00, RX_BUFFER_SIZE);
              _tcp_buffer_write_index = 0;
            }
          }
        }
        return true;
...
public:
    ...
    void modemStop(){
      uint8_t buffRX[RX_BUFFER_SIZE];
      cleanBuffer(buffRX, RX_BUFFER_SIZE);
      sendATCommand(buffRX, (FSH)TCP_CLOSE, (FSH)OK, (FSH)ERROR, 2000); 
